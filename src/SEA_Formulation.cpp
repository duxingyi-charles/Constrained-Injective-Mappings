...
double SEA_Formulation::compute_energy(const VectorXd &x) {
    ...
    // TGC
    double energy = tgc.compute_total_generalized_content(V);
    ...
    if (!fixed_boundary) {
        ...
        energy = energy + arc_segment_area - arc_occupancy;
    }
    // when boundary is fixed, we optimize total generalized content
...
double SEA_Formulation::compute_energy(const VectorXd &x, VectorXd &energy_list) {
    ...
    double tgc_energy = tgc.compute_total_generalized_content(V,generalized_content_list);
    ...
    if (!fixed_boundary) {
        ...
        energy_list.resize(generalized_content_list.size() + arc_curve_segment_area_list.size() + 1);
        int ii = 0;
        for (int i = 0; i < generalized_content_list.size(); ++i) {
            energy_list(ii) = generalized_content_list(i);
            ++ii;
        }
        ...
        return tgc_energy + arc_segment_area - arc_occupancy;
    }
    else {
...
double SEA_Formulation::compute_energy_with_gradient(const VectorXd &x, VectorXd &grad) {
    ...
    if (!fixed_boundary) {
        ...
        // full gradient
        Matrix2Xd m_grad = tgc_grad + arc_seg_grad - arc_occupancy_grad;
