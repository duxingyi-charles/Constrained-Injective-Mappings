...
void export_result(const std::string &output_file,
                   ...
                   const std::vector<std::vector<SubArc_Edge>> &edges_of_cell,
                   const std::vector<int> &windings)
{
    ...
    output["cell_edges"] = json::array();
    output["cell_edges_angle"] = json::array();
    output["cell_edges_raw_edge_id"] = json::array();
    for (const auto & edges : edges_of_cell) {
        json j_edges = json::array();
        json j_angles = json::array();
        json j_raw_ids = json::array();
        for (const auto & e : edges) {
            j_edges.push_back({e.id1, e.id2});
            j_angles.push_back(e.arc.get_arc_angle());
            j_raw_ids.push_back(e.parent_id);
        }
        output["cell_edges"].push_back(j_edges);
        output["cell_edges_angle"].push_back(j_angles);
        output["cell_edges_raw_edge_id"].push_back(j_raw_ids);
    }
...
//    }
    ...
    // output result
    export_result(args.output_file, pts, pEdges, is_intersection_point, edges_of_cell, windings);
