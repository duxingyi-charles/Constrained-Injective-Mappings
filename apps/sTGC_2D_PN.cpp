...
// Total Generalized Lifted Content (TGC) Projected-Newton solver for triangle meshes
// Total Lifted Content (TLC) was introduced in "Lifting Simplices to Find Injectivity", SIGGRAPH 2020.
...
// TLC and IsoTLC are special cases of TGC under different parameter settings:
...
public:
    ...
    sTGC_2D_Formulation formulation;
...
//        }
        ...
        if (record_last_non_flip)
        {
            ...
            compute_tri_mesh_singular_values(formulation.get_scaled_rest_vertices(), last_non_flip_V, F,
                                             last_non_flip_singular_values);
...
int main(int argc, char const* argv[])
{
    ...
    if (!succeed) {
        std::cout << "usage: sTGC_2D_PN [inputFile] [solverOptionsFile] [resultFile]" << std::endl;
        return -1;
    }
