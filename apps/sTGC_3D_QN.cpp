...
// Total Generalized Lifted Content (TGC) Quasi-Newton solver for tetrahedral meshes
...
public:
    ...
    SolverOptionManager() :
            form("Tutte"), alpha(1),
            lambda1(1.0/6.0), lambda2(0.), k(1.), // 6 * k^(1/3) * lambda1 + 2 * k * lambda2 = 1
            scale_rest_mesh(false), subtract_total_signed_content(true),
    ...
    // whether to subtract total signed content
    ...
    void printOptions()
    {
        ...
        std::cout << "subtract_total_signed_content:\t" << subtract_total_signed_content << "\n";
...
//            in_file >> theta;
            ...
            in_file >> optName;
            if (optName != "subtract_total_signed_content") {
                abnormal = "subtract_total_signed_content";
                break;
            }
            in_file >> subtract_total_signed_content;
...
{
public:
    Optimization_Data(const Matrix3Xd& restV,
                      const Matrix3Xd& initV,
                      const Matrix4Xi& restF,
...
//            locally_injective_Found(false), first_locally_injective_iteration(-1),
...
//            record_nb_winded_interior_vertices(false),
            ...
            is_boundary_vertex(initV.cols(), false)
    {
        ...
        const auto& boundary_triangles = formulation.get_boundary_triangles();
        for (const auto& f : boundary_triangles) {
            is_boundary_vertex[f[0]] = true;
            is_boundary_vertex[f[1]] = true;
            is_boundary_vertex[f[2]] = true;
        }
    ...
    sTGC_3D_Formulation formulation;
...
//    Matrix3Xd first_locally_injective_V;
    bool non_flip_Found;
    int first_non_flip_iteration;
    Matrix3Xd first_non_flip_V;
...
//            if (i == "nbWindVert") record_nb_winded_interior_vertices = true;
    ...
    void record()
    {
        if (record_vert) vertRecord.emplace_back(formulation.get_V());
        ...
        if (record_minArea)
        {
            double minA = compute_min_signed_mesh_volume(formulation.get_V(), F);
...
int main(int argc, char const* argv[])
{
    ...
    bool succeed = importData3D(dataFile, restV, initV, F, handles);
    if (!succeed) {
        std::cout << "usage: sTGC_3D_QN [inputFile] [solverOptionsFile] [resultFile]" << std::endl;
        return -1;
    }
...
//    options.printOptions();
    ...
    Optimization_Data data(restV, initV, F, handles, options.form,options.alpha,
                           ...
                           options.scale_rest_mesh, options.subtract_total_signed_content);
...
//    VectorXd x0 = data.x0;
//    VectorXd g0;
//    double energy0 = data.formulation.compute_energy_with_gradient(x0, g0);
//    double delta = 1e-6;
